...
contract AdminInsurancePolicy {
    ...
    // Function to archive (deactivate) a policy with validation
    function archivePolicy(
        uint256 _policyId
    ) public sameAdmin policyExists(_policyId) policyIsActive(_policyId) {
        Policy storage policy = policies[_policyId];
        policy.isActive = false;

        emit PolicyArchived(_policyId);
    }

    // Function to retrieve a policy's details
    function getPolicy(
        uint256 _policyId
    )
        public
